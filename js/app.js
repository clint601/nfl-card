...
//     confTeams = data.filter(team => team.conference == conf)
...
// let afcTeams = data.filter(team => team.conference == 'atc')
...
data.forEach(obj => {
    ...
    // had to take care of some person business so couldnt fix the issue 

    let bestPlayer = []


    const getBestPlayer =(arr, player)=> {
        arr.forEach(item => item.notablePlayers == player ? bestPlayer = [...bestPlayer, item]: null)
    }

    getBestPlayer(data, notablePlayers)
